...
module systolic_4x4 (
    ...
    systolic_pe pe03 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h02),    .b_in(feed_b3),  .a_out(),      .b_out(b_v03), .acc(acc03));
    ...
    systolic_pe pe13 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h12),    .b_in(b_v03),    .a_out(),      .b_out(b_v13), .acc(acc13));
    ...
    systolic_pe pe23 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h22),    .b_in(b_v13),    .a_out(),      .b_out(b_v23), .acc(acc23));

    // -------------------------------------------------------------------------
    // Row 3
    // -------------------------------------------------------------------------
    systolic_pe pe30 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(feed_a3),  .b_in(b_v20),    .a_out(a_h30), .b_out(),      .acc(acc30));
    systolic_pe pe31 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h30),    .b_in(b_v21),    .a_out(a_h31), .b_out(),      .acc(acc31));
    systolic_pe pe32 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h31),    .b_in(b_v22),    .a_out(a_h32), .b_out(),      .acc(acc32));
    systolic_pe pe33 (.clk(clk), .rst(rst), .clk_en(run_en), .clear(clear_pe), .a_in(a_h32),    .b_in(b_v23),    .a_out(),      .b_out(),      .acc(acc33));
